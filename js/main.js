...
function pcPlays(){
    ...
    setTimeout(() => {
        let played = false;
        const opcions = checkIfCanWin();

        if(opcions.length > 0){
            const bestOpcion = opcions[0];
            for(let i = 0; i < bestOpcion.length; i++){
                if(bestOpcion[i].value == 0){
                    const posi = bestOpcion[i].i;
                    const posj = bestOpcion[i].j;
                    board[posi][posj] = 'X';
                    played = true;
                    break;
                }
...
function checkIfCanWin(){
    const arr = JSON.parse(JSON.stringify(board)); //para copiar arrays bidimensionales
